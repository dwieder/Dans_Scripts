...
uw_15_scholars = '''
    ...
    select ca.scholar_id
    from class_assignment ca
    inner join school_class sc on ca.school_class_id=sc.id
    where sc.school_id=11391
    and ca.start_date between '2014-08-10' AND '2015-06-10'
    and ca.scholar_id not in (6141,	7076,	10427,	13840,	18978,	22213,23062,	24997,	25480,	25661,	28894,	29016,	34396,	34990,	36315,	43589,	47708,	49367,	2300033247,	2300037223)
    group by ca.scholar_id;'''

uw = read_sql(uw_15_scholars,conn_pgsql)
uw = uw['scholar_id'].tolist()
len(uw)